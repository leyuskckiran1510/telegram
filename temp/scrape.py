...
class Run:
    cok={    'accept': "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
    ...
    def youtube(self,url):
        ...
        try:
            ...
            return {"error":"This video requires age confirmation so; please login to youtube by yourself; can't be downloded"}
    ...
    def tiktok(self,url):
        ...
        try:
            ...
            url={"video":{"url":a},"audio":{"Audio and video files are in same link as file is small and easy to stream":"Audio is by default in video link"}}
            return url
        except KeyError as e:
            return {"error":"This video requires age confirmation so; please login to youtube by yourself; can't be downloded"}
        except IndexError as e:
    ...
    def instagram(self,url):
        ...
        try:
            ...
            st = eval(f"unquote('{d}')")
            url={"video":{"url":st},"audio":{"Audio and video files are in same link as file is small and easy to stream":"Audio is by default in video link"}}
            return url
        except IndexError as e:
            return {"error":"Can't use same url multiple time in Short Time Interval Instagram bans IP Sorry try next url."}
        except Exception as e:
            id = uuid.uuid1()
            with open("errors.txt","a") as fl:
                fl.write(f"id : {e}"+"\n"+"*"*10+"\n")
    ...
    def urls(self,url,inline=False):
        try:
            ...
            return ("Typing...")
        dic = {'youtube':self.youtube,'youtu':self.youtube,'tiktok':self.tiktok,'facebook':self.facebook,'fb':self.facebook,"instagram":self.instagram}
        for i in domain:
            if i in dic.keys():
                if not inline:
                    return dic[i](url)
